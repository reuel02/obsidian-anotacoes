...
topic: Formatação e Estilização de Documento
---

**user**: Formate e estilize este documento de estudo
[Context: Notes: AWS.md]
[Timestamp: 2026/06/18 15:02:22]

**ai**: Aqui está uma versão organizada e estilizada do seu documento de estudo sobre o deploy de aplicações na AWS, utilizando uma estrutura de guia técnico:
...
Automatização via **GitHub Actions** disparada a cada *push* na branch `main`.
...
- **FinOps:** Configuração de *Budget* de "Zero Spend" ($0.01) para evitar surpresas na fatura.
...
*Dica: Use `docker ps` para listar contêineres e `docker logs <nome>` para depuração.*
